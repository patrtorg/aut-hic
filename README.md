...
![split view](https://raw.githubusercontent.com/otakustay/@patrtorg/aut-hic/master/screenshots/split-view.png)

### Unified view

![unified view](https://raw.githubusercontent.com/otakustay/@patrtorg/aut-hic/master/screenshots/unified-view.png)

### Optimized selection

![select only one side](https://raw.githubusercontent.com/otakustay/@patrtorg/aut-hic/master/screenshots/single-side-selection.png)
...
```shell
npm install --save @patrtorg/aut-hic
```

Starting from `3.1.0`, you can `import {Diff} from '@patrtorg/aut-hic/esm` to reference an unminified ESM module.
...
![Normal sequence behavior](https://raw.githubusercontent.com/otakustay/@patrtorg/aut-hic/master/screenshots/sequence-normal.png)
...
![Normal sequence behavior](https://raw.githubusercontent.com/otakustay/@patrtorg/aut-hic/master/screenshots/sequence-zip.png)
...
```tsx
import {parseDiff, Diff, Hunk} from '@patrtorg/aut-hic';
...
import {Diff, Hunk, Decoration} from '@patrtorg/aut-hic';
...
In some cases we need functions like commenting on change, `@patrtorg/aut-hic` provides an extensible solution called **widget** to archive such scenarios.
...
```javascript
import {parseDiff, getChangeKey, Diff} from '@patrtorg/aut-hic';
...
The basic theme of `@patrtorg/aut-hic` is simply "picked" from github, with some additional colors for column diffs, the style is located at [`@patrtorg/aut-hic/style/index.css`](https://unpkg.com/browse/@patrtorg/aut-hic/style/index.css), you can simply import this file in your project.

### CSS variables

`@patrtorg/aut-hic` includes a bunch of css variables (custom properties) to customize the presentation of diff in [supported browsers](https://caniuse.com/#feat=css-variables), these includes:
...
You can override styles on certain css classes to customize the appearance of `@patrtorg/aut-hic`, here is a list of css classes generated by component:
...
The `@patrtorg/aut-hic` also ships with several common enhancers:
...
`@patrtorg/aut-hic` comes with some utility functions to help simplify common issues:
...
- Add a `@patrtorg/aut-hic/parse` module providing 2 named exports: `parseDiff` and `addStubHunk`, this can be used on web workers.
...
- Fix a bug where incorrect `sideEffects` config in `package.json` can make webpack tree shaking css of `@patrtorg/aut-hic`.
